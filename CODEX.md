...
- `codex/ProjectSummary.md` — architecture, models, structure
- `codex/Progress.md` — current phase and state

Load on demand only:
- `codex/BuildFlow.md` — when entering a new phase or running `/phase-check`
- `codex/G0_questionnaire.md` — only while G0 is incomplete
...
- Any codex/ file contains unfilled placeholders: `[PLACEHOLDER]`, `[TO_BE_FILLED]`, `[G0.6 fills]`, `[NAME]`, `[DATE]`, `[PROJECT_SCOPES]`, `[TDD_TARGETS]`, `[DOCKER_PHASE]`, `[CI_PHASE]`, `[PROJECT_RED_LINES]`, `[APP_NAME]`, `[FILLED BY G0.6]`
- `_fill_manifest.md` contains bracketed placeholder values

**If G0 incomplete →** load `codex/G0_questionnaire.md`, run from earliest incomplete sub-gate.
...
Phase-specific proof requirements live in `codex/BuildFlow.md` under each phase's **Proof** line.
